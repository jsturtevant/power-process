...
pub fn anon_pipe(ours_readable: bool, their_handle_inheritable: bool) -> io::Result<Pipes> {
    ...
    unsafe {
        ...
        loop {
            ...
            ours = Handle::from_raw_handle(handle as RawHandle);
            break;
        }
...
impl AnonPipe {
    ...
    pub fn read(&self, buf: &mut [u8]) -> io::Result<usize> {
        let result = unsafe {
            let len = std::cmp::min(buf.len(), c::DWORD::MAX as usize) as c::DWORD;
    ...
    unsafe fn alertable_io_internal(
        ...
        let mut overlapped: c::OVERLAPPED = std::mem::zeroed();
